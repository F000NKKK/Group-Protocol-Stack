...
GBPFrame {
  ...
  uint8    pf;               // опционально; опускается при значении 0 (CBOR) для обратной совместимости
}
```

Поле `pf` (payload format) идентифицирует кодек, применённый для кодирования субпротокольного сообщения внутри `encrypted_payload` до AEAD-запечатывания. Реестр значений см. в §6.5 профиля сериализации (schemas.ru.md). Получатели MUST трактовать отсутствующее поле `pf` как `0` (CBOR).
