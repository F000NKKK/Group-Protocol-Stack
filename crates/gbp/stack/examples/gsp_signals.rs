...
use gbp_stack::{
    ControlOpcode, Event, GroupNode, GspClient, MlsContext, PayloadCodec, SignalType, StreamType,
};
use openmls::prelude::{
...
fn cbor_map1(k: u32, v: u32) -> Vec<u8> {
    let mut b = vec![0xA1];
    b.extend(cbor_uint(k));
    b.extend(cbor_uint(v));
    b
}
fn cbor_map2(k0: u32, v0: u32, k1: u32, v1: u32) -> Vec<u8> {
    let mut b = vec![0xA2];
    b.extend(cbor_uint(k0));
    b.extend(cbor_uint(v0));
    b.extend(cbor_uint(k1));
    b.extend(cbor_uint(v1));
    b
}
